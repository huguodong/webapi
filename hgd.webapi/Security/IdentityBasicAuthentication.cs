...
namespace hgd.webapi.Security
{
    public class IdentityBasicAuthentication : IAuthenticationFilter
    {
        ...
        public Task AuthenticateAsync(HttpAuthenticationContext context, CancellationToken cancellationToken)
        {
            ...
            if (isValid)
            {
                var identity = new ClaimsIdentity("jwt", "userName", "roles");//只要ClaimsIdentity设置了authenticationType，authenticated就为true，后面的authority根据authenticated=true来做权限
            ...
            {
                context.ErrorResult = new ResponseMessageResult(Utils.toJson(HttpStatusCode.OK, new { code = HttpCode.AuthenticationRequired, msg = errMsg }));
